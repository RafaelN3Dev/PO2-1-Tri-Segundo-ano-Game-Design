/**
 * Atari Survival - Final Build with all Mechanics (1-6)
 */
...
class Player {
    ...
    update(keys, deltaTime, zones) {
        ...
        if (ammo < MAX_AMMO) {
            ...
            if (lastRegenTime >= REGEN_TIME) { ammo++; lastRegenTime = 0; updateUI(); }
        }
    }
    draw(ctx) {
        ...
        ctx.fillStyle = '#1e90ff'; ctx.fillRect(10, -5, 8, 10); // Muzzle
        ctx.rotate(-this.angle);
...
class Bullet {
    ...
    update() {
        ...
        if (this.x < 0 || this.x > CANVAS_WIDTH - 6) { this.dx *= -1; this.bounces++; }
        if (this.y < 0 || this.y > CANVAS_HEIGHT - 6) { this.dy *= -1; this.bounces++; }
...
class Enemy {
    constructor(x, y) {
        this.x = x; this.y = y; this.speed = 0.8; this.active = true; this.hp = 1;
...
function gameLoop(ts) {
    ...
    if (gameState === 'PLAYING') {
        ...
        updateUI(); // Keep dash timer updated
    }
